...
async function getData() {
  try {
    ...
    const [servicesSnap, gallerySnap] = await Promise.all([
      db.collection('services').where('active', '==', true).orderBy('order', 'asc').limit(4).get(),
      db.collection('galleryImages').orderBy('order', 'asc').limit(6).get(),
    ]);

    const services: Service[] = servicesSnap.docs.map((doc) => ({
      id: doc.id,
...
export default async function HomePage() {
  ...
  return (
    <div className="flex flex-col">
      {/* Hero */}
      <section className="relative bg-[#0f0f0f] text-white min-h-[80vh] flex items-center overflow-hidden">
        <div className="absolute inset-0 opacity-5">
          <div className="absolute inset-0" style={{ backgroundImage: 'repeating-linear-gradient(45deg, #c0392b 0, #c0392b 1px, transparent 0, transparent 50%)', backgroundSize: '20px 20px' }} />
        </div>
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-20 grid md:grid-cols-2 gap-12 items-center">
          <div>
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-[#c0392b] text-sm font-medium uppercase tracking-widest">
            ...
            <h1 className="font-display text-6xl sm:text-7xl lg:text-8xl leading-none mb-6">
              ...
              <span className="text-[#c0392b]">&apos;s</span>
              <br />
            ...
            <div className="flex flex-wrap gap-4">
              <LinkButton
                href="/booking"
                className="bg-[#c0392b] hover:bg-[#a93226] text-black font-semibold px-8 py-6 text-base rounded-md border-transparent"
              >
                Book Now
              </LinkButton>
              <LinkButton
          ...
          <div className="hidden md:grid grid-cols-2 gap-4">
            {[
              ...
              <div key={label} className="bg-white/5 border border-white/10 rounded-xl p-5 backdrop-blur-sm">
                <Icon className="h-7 w-7 text-[#c0392b] mb-3" />
      ...
      {services.length > 0 && (
        <section className="py-16 px-4 sm:px-6 max-w-6xl mx-auto w-full">
          <div className="flex items-end justify-between mb-8">
            ...
            <Link href="/services" className="text-sm text-[#c0392b] hover:underline font-medium">
              View all →
            </Link>
          </div>
          ...
          <div className="text-center mt-8">
            <LinkButton
              href="/booking"
              className="bg-[#c0392b] hover:bg-[#a93226] text-black font-semibold px-8 border-transparent"
            >
              Book an Appointment
            </LinkButton>
          </div>
      ...
      {gallery.length > 0 && (
        <section className="py-16 px-4 sm:px-6 max-w-6xl mx-auto w-full">
          <div className="flex items-end justify-between mb-8">
            ...
            <Link href="/gallery" className="text-sm text-[#c0392b] hover:underline font-medium">
              Full gallery →
            </Link>
          </div>
      ...
      {/* Final CTA */}
      <section className="bg-[#c0392b] py-16 px-4 text-center">
        <h2 className="font-display text-5xl tracking-wide text-black mb-3">Ready for a Fresh Cut?</h2>
        <p className="text-black/70 mb-8 text-lg">
          Book your appointment online in seconds. Walk-ins also welcome.
        </p>
        <LinkButton
          href="/booking"
          className="bg-[#0f0f0f] hover:bg-[#1a1a1a] text-white font-semibold px-10 py-6 text-base rounded-md border-transparent"
        >
          Book Now →
        </LinkButton>
      </section>
